pipeline {
  ...
  parameters {
    ...
    string(name: 'git_mood2anime_db', defaultValue: 'https://github.com/lain0x15/mood2anime_db.git', description: 'Git repo with data for import')
    ...
    stage('test') {
      steps {
        ansiblePlaybook(credentialsId: "${params.name_ssh_secret}",
          ...
          inventory: '${ip_vm},',
          extras: "-e DJANGO_CSRF_TRUSTED_ORIGINS='https://${params.dns_name_web_site}' -e DJANGO_WEBSITE_DNS_NAME='${params.dns_name_web_site}' -e docker_image='${params.docker_image}' -e metrics_html='${params.metrics_html}' -e git_mood2anime_db='${git_mood2anime_db}'"
        )
      }
    }
  }
